...
public class FragmentFifth extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (binding == null) {
            binding = DataBindingUtil.inflate(inflater, R.layout.activity_fifth, container, false);
            binding.img.setBackground(drawables);
        }
        if (update) {
            if (drawables != null) {
                for (int i = 0; i < 6; i++) {
                    Drawable d = BitmapDrawable.createFromPath(DataModel.getInstance().ldPath + (5 - i) + ".gif");
                    if (d != null)
